...
id=?',
    ...
    if ($_POST["status"] === "paid") {
        $pdo->prepare(
            "UPDATE payments SET status='paid', paid_at=NOW() WHERE
order_id=?",
        )->execute([$_POST["id"]]);
    }
...
tracking_number=COALESCE(NULLIF(?,''), tracking_number), shipped_at=NOW() WHERE
order_id=?",
        )->execute([$_POST["tracking_number"] ?? "", $_POST["id"]]);
    }
}
$orders = $pdo
    ->query(
        'SELECT o.*, p.status payment_status, s.status
shipment_status, s.tracking_number FROM orders o LEFT JOIN payments p ON
p.order_id=o.id LEFT JOIN shipments s ON s.order_id=o.id ORDER BY o.created_at
DESC',
    )
    ->fetchAll();
?>
    ...
    <div class="admin-shell">
      ...
      <main>
        <div class="glass-panel">
          ...
          <p>Status order terhubung dengan pembayaran dan pengiriman.</p>
        </div>
